...
class AdicionarPedidoScreen extends StatefulWidget {
  const AdicionarPedidoScreen({super.key});

  @override
  State<AdicionarPedidoScreen> createState() => _AdicionarPedidoScreenState();
}

class _AdicionarPedidoScreenState extends State<AdicionarPedidoScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            _Secao(titulo: 'Dados do Pedido', children: [
              ...
              // Seleção de cliente
              GestureDetector(
                onTap: () => _selecionarCliente(context, clienteVm.clientes),
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: const Color(0xFF2A2A3E),
            ...
            _Secao(titulo: 'Itens', children: [
              ...
              GestureDetector(
                onTap: () => _adicionarProduto(context, produtoVm.produtos),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Row(children: [
              ...
              if (_itens.isNotEmpty) ...[
                ...
                ..._itens.map((item) => _ItemPedidoTile(
                  item: item,
                  onRemover: () => setState(() => _itens.remove(item)),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  vm.carregando ? 'Salvando...' : 'Salvar Pedido',
                  style: const TextStyle(
  ...
  // Abre bottomsheet para selecionar cliente
  void _selecionarCliente(BuildContext context, List<Cliente> clientes) {
  ...
  // Abre bottomsheet para selecionar produto
  void _adicionarProduto(BuildContext context, List<Produto> produtos) {
    showModalBottomSheet(
      context: context,
      backgroundColor: const Color(0xFF1E1E2E),
      isScrollControlled: true,
  ...
  void _salvar() async {
    ...
    final ok = await context.read<PedidoViewModel>().adicionarPedido(pedido);
    if (ok && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
      ...
      child: Row(
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('R\$ ${item.precoUnitario.toStringAsFixed(2)} un.',
                    style: const TextStyle(
                        color: Color(0xFF9CA3AF), fontSize: 12)),
              ],
            ),
          ...
          Row(children: [
            GestureDetector(
              onTap: () {
                if (item.quantidade > 1) onAlterarQtd(item.quantidade - 1);
                else onRemover();
              },
              child: Container(
